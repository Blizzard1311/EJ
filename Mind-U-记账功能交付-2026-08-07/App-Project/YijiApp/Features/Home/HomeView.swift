...
struct HomeView: View {
    ...
    private struct StorageContainerPickerView: View {
        ...
        private func storageContainerTint(_ container: StorageContainer) -> Color {
            switch container {
            case .medicineKit:
                Color(red: 0.91, green: 0.56, blue: 0.53)
            case .documentPouch:
                Color(red: 0.46, green: 0.66, blue: 0.73)
            case .jewelryBox:
                Color(red: 0.62, green: 0.57, blue: 0.72)
            case .digitalBox:
                Color(red: 0.52, green: 0.70, blue: 0.75)
            case .wardrobe:
                Color(red: 0.53, green: 0.63, blue: 0.54)
            case .storageBox:
                Color(red: 0.75, green: 0.64, blue: 0.42)
            case .drawer:
                Color(red: 0.60, green: 0.64, blue: 0.56)
            case .bag:
                Color(red: 0.64, green: 0.56, blue: 0.67)
            }
        }
    }
    ...
    private var storageHeaderSection: some View {
        Section {
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack(spacing: 8) {
                    ...
                    TextField("搜索物品或位置", text: $appModel.searchText)
                        .textInputAutocapitalization(.never)
                        .submitLabel(.search)
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                if group.records.isEmpty {
                    ...
                    ForEach(Array(group.records.prefix(2).enumerated()), id: \.element.id) { _, record in
                    ...
                    if group.records.count > 2 {
                        Text("还有 \(group.records.count - 2) 件")
...
struct CalendarView: View {
    ...
    private var dayEntriesCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            sectionHeader(title: "当天记录", countText: "\(selectedDateEntries.count) 条")
    ...
    private var calendarLegend: some View {
        HStack(spacing: 16) {
            ...
            HStack(spacing: 6) {
                Image(systemName: "cloud.sun.fill")
                    .font(.caption)
                    .foregroundStyle(Color(red: 0.40, green: 0.54, blue: 0.60))
                Text("天气")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
