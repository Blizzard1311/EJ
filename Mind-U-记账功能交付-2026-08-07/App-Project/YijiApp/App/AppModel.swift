...
final class AppModel: ObservableObject {
    ...
    func updateExpenseDraftText(_ text: String) {
        ...
        expenseDraft = expenseParser.parse(text, categories: expenseCategories)
    }
    ...
    func saveExpenseDraft() async {
        guard let draft = expenseDraft else {
            showTransientStatus("请先说出消费内容和金额。")
            return
        }
        let title = draft.title.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !title.isEmpty, draft.amount > 0 else {
        ...
        do {
            let snapshot = try await repository.saveExpense(expense)
            applyExpenseSnapshot(snapshot)
            clearExpenseDraft()
            showTransientStatus("已记账")
            await refreshCloudSyncStatus()
        } catch {
            showPersistentStatus("记账保存失败：\(error.localizedDescription)")
        }
    }
    ...
    func deleteExpense(id: UUID) async {
        do {
            ...
            showPersistentStatus("删除帐目失败：\(error.localizedDescription)")
        }
    }
    ...
    func addCustomExpenseCategory(named name: String, colorHex: String = "D9C9E8") async -> String? {
        ...
        guard !expenseCategories.contains(where: { $0.name.caseInsensitiveCompare(trimmed) == .orderedSame }) else {
            showTransientStatus("这个类别已经存在。")
            return nil
        }

        let definition = ExpenseCategoryDefinition(customName: trimmed, colorHex: colorHex)
        let updated = expenseCategories + [definition]
        do {
            let snapshot = try await repository.updateExpenseCategories(updated)
            applyExpenseSnapshot(snapshot)
            await refreshCloudSyncStatus()
            return definition.id
        } catch {
            showPersistentStatus("新增类别失败：\(error.localizedDescription)")
            return nil
        }
    }
    ...
    private func applyExpenseTranscript(_ transcript: String) {
        ...
        expenseDraft = expenseParser.parse(trimmed, categories: expenseCategories)
    }
